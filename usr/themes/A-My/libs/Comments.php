...
class Comments
{
    ...
    public static function commentReply($archive)
    {
        if ($archive->allow('comment')) {
            if (isset($GLOBALS['page']) && $GLOBALS['page'] == 'page-cross'){
                echo "
                <script type=\"text/javascript\">
                /** page-cross comment create && cancel */
                function createReply(coid, author) {
                    $('.Comments-from').addClass('Comments_publisher'); 
                  ...
                  $('#cancelReply').html(\"<span class='flex items-center justify-center'>@\"+ author +\"<i class='ri-close-line ri-xl'>\").css('display', 'inline-block');
                  $(\"html, body\").animate({
                    scrollTop: $('.comment-form').offset().top + -80 + \"px\"
                  }, {
                    duration: 250,
                    easing: \"linear\"
                  });
                  ...
                  tip.open({
                    content: '<div class=\"flex justify-center items-center\"><iconpark-icon name=\"info\" class=\"mr-1\" color=\"#2b6cb0\"></iconpark-icon>正在回复:'+ author +'</div>',
                    skin: 'msg',
                    time: 3,
                    anim: 'down',
                    shade: false
                ...
                function cancelReply() {
                  ...
                  tip.open({
                    content: '<div class=\"flex justify-center items-center\"><iconpark-icon name=\"info\" class=\"mr-1\" color=\"#2b6cb0\"></iconpark-icon>已取消回复</div>',
                    skin: 'msg',
                    time: 3,
                    anim: 'down',
                    shade: false
                ...
                <script type=\"text/javascript\">
                    ...
                    $('#textarea').focus();
                    tip.open({
                        content: '<div class=\"flex justify-center items-center\"><iconpark-icon name=\"info\" class=\"mr-1\" color=\"#2b6cb0\"></iconpark-icon>正在回复：' + author,
                        skin: \"msg\",
                        time: 3,
                        anim: \"down\",
                        shade: false
    ...
    {
        $db = Typecho_Db::get();
        $result = $db->fetchAll($db->select('author', 'mail', 'text')->from('table.comments')->limit('3')
