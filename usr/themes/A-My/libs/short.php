...
class Editor
{
    ...
    {
?>
        <link rel="stylesheet" type="text/css" href="<?php Utils::SrcCdnDir();?>src/css/plugins/APlayer.min.css">
        <link rel="stylesheet" type="text/css" href="<?php Utils::SrcCdnDir();?>src/css/plugins/prism-onedark.min.css">
        ...
        <script>
            window.JoeConfig = {
                ...
                playerAPI: '<?php Helper::options()->CustomPlayer ? Helper::options()->CustomPlayer() : Helper::options()->themeUrl('library/player.php?url=') ?>',
...
function _parseContent($post, $login)
{
    ...
    $pattern = '/\<?[p]?[br]*\>\<img src="(.*?)" alt="(.*?)" title="(.*?)">/s';
    if (Helper::options()->imgAlt){
        $replacement = '<a class="masonry-item" data-caption="$2"><img class="lazy ViewImage" src="$1" alt="$2" data-src="$1"><span class="masonry-alt">$2</span></a>';
    } else {
        $replacement = '<a class="masonry-item" data-caption="$2"><img class="lazy ViewImage"src="$1" alt="$2" data-src="$1"></a>';
    }
