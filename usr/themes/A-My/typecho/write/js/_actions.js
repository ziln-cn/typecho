		...
		$.ajax({
			...
			success: res => {
				...
				for (let key in res) {
					...
					listsStr += `<div class="lists ${key === '泡泡' ? 'active' : ''}" data-show="${key}">${arr.map(item => `<div class="lists-item" data-text="${item.data}">${key === '颜文字' ? item.icon : `<img src="${item.icon}">`}</div>`).join(' ')}</div>`;
				}
				this._openModal({
					title: '普通表情',
					hasFooter: false,
	...
	handleAbtn(cm) {
		this._openModal({
			...
			innerHtml: `
				<div class="fitem">
					<label>按钮图标</label>
					<input autocomplete="off" name="icon" placeholder="请输入mdi图标，例：mdi-download"/>
				</div>
				<div class="fitem">
					<label>图标大全</label>
					<a href="https://materialdesignicons.com/" target="_blank">materialdesignicons</a>
				</div>
				<div class="fitem">
					<label>按钮颜色</label>
