...
<div class="container flex flex-wrap flex-1 px-0 sm:px-2 mx-auto mt-3 md:flex-no-wrap">
  <main class="flex-1 w-full" id="mobo" style="width: 0;">
    <div class="box sm:box mb-3 p-3">
      <div class="post_thumbnail-custom flex mb-3 h-48 relative rounded-md overflow-auto">
        ...
        <span class="article-headline-p w-full flex justify-center text-white tracking-wider m-auto flex-col z-10 px-5">
            <div class="movies-content">
                <div class="info flex">
                    <a href="<?php $this->permalink() ?>" class="movies-img overflow-auto rounded-r-md" aria-label="<?php $this->title() ?>">
                    ...
                    <div class="flex-1 ml-5 grid items-center">
                        <div class="ml-3 grid items-center h-full">
                            <h3 class="ell text-white">《<?php $this->title() ?>》</h3>
                            <p class="m-0">导演: <?php $this->fields->movies_author(); ?></p>
                            <p class="m-0">上映时间: <?php $this->fields->movies_time(); ?></p>
                            <p class="m-0">评分: <?php $this->fields->movies_score(); ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <joe-tabs class="douban_body">
            <span class="_temp" style="display: none">
                <br>{tabs-pane label="观后感"}<br>
                    ...
                    <?php endif; ?>
                <br>{/tabs-pane}<br>{tabs-pane label="介绍"}
                    <?php $this->fields->movies_excerpt(); ?>
                <p>{/tabs-pane}<br></p>
            </span>
        </joe-tabs>
      </section>

      <span class="py-3 post-tags">
        <?php $this->tags(' ', true, ''); ?>
      </span>
